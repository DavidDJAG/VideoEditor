...
{
    private static readonly string[] DefaultConvertContainerOptions = ["mp4", "mkv", "mov", "avi", "webm", "m4a", "mp3", "wav", "flac", "aac", "ogg", "opus"];
    ...
    public ObservableCollection<string> ConvertContainerOptions { get; } = new();
    ...
    {
        SyncCollection(ConvertContainerOptions, DefaultConvertContainerOptions);
    ...
    {
        ConvertContainer = EnsureSelectedOption(ConvertContainer, ConvertContainerOptions, DefaultConvertContainerOptions, "mp4");
    ...
    private string BuildConvertQueueJobName(string inputPath, string outputPath)
    {
        ...
        var targetName = string.IsNullOrWhiteSpace(outputPath)
            ? $"output.{NormalizeContainer(ConvertContainer)}"
    ...
    private List<string> BuildConvertPresetValidationErrors(ConvertOptions convertOptions, List<string> parseErrors)
    {
        ...
        {
            if (!ContainsIgnoreCase(_convertCapabilitiesSnapshot.Muxers, normalizedContainer))
            {
    ...
    private List<string> BuildConvertAdvisories(ConvertOptions convertOptions)
    {
        ...
        if (!string.IsNullOrWhiteSpace(ConvertOutputPath))
        {
            ...
            if (!string.IsNullOrWhiteSpace(outputExtension)
                ...
                && !string.Equals(outputExtension, normalizedContainer, StringComparison.OrdinalIgnoreCase))
            {
    ...
    private static bool SupportsFastStart(string? container)
        => NormalizeContainer(container ?? string.Empty) is "mp4" or "mov" or "m4v";
    ...
    private static IEnumerable<string> BuildContainerOptions(ToolchainCapabilitiesSnapshot snapshot)
        => BuildOrderedCapabilityOptions(snapshot.Muxers, DefaultConvertContainerOptions);
    ...
    private string BuildSuggestedConvertOutputPath(string? inputPath, string? preferredDirectory, ConvertOptions? convertOptions)
    {
        ...
        var targetExtension = string.IsNullOrWhiteSpace(normalizedContainer) ? string.Empty : $".{normalizedContainer}";
    ...
    private string BuildConvertOutputFilter()
    {
        ...
        foreach (var container in ConvertContainerOptions.Distinct(StringComparer.OrdinalIgnoreCase))
        {
            var label = container.ToUpperInvariant();
            entries.Add($"{label} (*.{container})|*.{container}");
        }
    ...
    private static string BuildConvertInputSourceSummary(MediaProbeResult probeResult)
    {
        ...
        {
            parts.Add($"Container: {probeResult.Container}");
        }
